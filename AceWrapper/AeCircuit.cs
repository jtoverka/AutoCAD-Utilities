...
namespace AceWrapper
{
	...
	public class AeCircuit : IDisposable
	{
		...
		/// </summary>
		public Dictionary<string, Attributes> BlockKeys { get; set; } = null;
		...
		{
			blockId.Validate(Rx.GetClass<BlockTableRecord>(), true);
		...
		private void UpdateCircuitSort(AeCircuit circuit)
		{
			...
			{
				if (objectId.IsMatch<Line>())
				{
					Attributes lineData = AeXData.Default();
					lineData.GetAttributes(objectId);

					if (!lineData.ContainsKey("WNPTR"))
						continue;

					ObjectId wireNoId = new();
					try { wireNoId = database.GetObjectId(false, new Handle(Convert.ToInt64(lineData["WNPTR"].Text, 16)), 0); }
					catch { }
					...
					attributes.GetAttributes(wireNoId);

					if (!(attributes.ContainsKey("WIRENO") && !attributes["WIRENO"].ObjectId.IsNull))
						continue;

					circuit.wires.Add(objectId, attributes["WIRENO"].ObjectId);
				}
				else if (objectId.IsMatch<BlockReference>())
				{
					...
					Attributes attributes = AeXData.Default();
					attributes.GetAttributes(objectId);

					circuit.blockAttributes.Add(objectId, attributes);
					circuit.blockReferences.Add(objectId);

					if (attributes.ContainsKey("LINKTERM"))
						circuit.linkTermCollection.Add(objectId);

					if (wireNo.IsMatch(blockReference.Name) && attributes.ContainsKey("WIRENO") && !attributes["WIRENO"].ObjectId.IsNull)
						circuit.wireNoAttributes.Add(attributes["WIRENO"].ObjectId, attributes["WIRENO"].Text);

					foreach (var item in attributes)
					{
						string attributeTag = item.Key;
						if ((terminal.IsMatch(attributeTag) || attributeTag.Equals("WIRENO")) && !item.Value.ObjectId.IsNull)
							circuit.terminalAttributes.Add(item.Value.ObjectId);
					}
				}
				else if (objectId.IsMatch<DBText>())
				{
					using DBText text = transaction.GetObject(objectId, OpenMode.ForRead) as DBText;
					circuit.dBText.Add(objectId, text.TextString);
				}
				else if (objectId.IsMatch<Dimension>())
				{
		...
		public void UpdateCircuitKeys()
		{
			...
			{
				foreach (KeyValuePair<ObjectId, Attributes> block in blockAttributes)
				{
					Attributes attributes = block.Value;
					Attributes write = new();

					foreach (KeyValuePair<string, DataPoint> attribute in attributes)
					{
		...
		public void UpdateLinkTerms()
		{
			...
			{
				attributes["LINKTERM"] = new(XDataType.ASCII, Guid.NewGuid().ToString().ToUpper());
				attributes.SetAttributes(id);
			}
		}
		...
		private ObjectId FindWireNoAttribute(ObjectId attributeId)
		{
			...
			if (attribute.Tag.Equals("WIRENO"))
			{
				...
				Attributes attributes = AeXData.Default();
				attributes.GetAttributes(attribute.OwnerId);
				foreach (var item in attributes)
				{
					if (!(terminal.IsMatch(item.Key) && !item.Value.ObjectId.IsNull))
						continue;

					ObjectId id = FindWireNoAttribute(item.Value.ObjectId);
