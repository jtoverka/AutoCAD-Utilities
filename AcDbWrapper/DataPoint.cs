...
namespace AcDbWrapper
{
	...
	public sealed class DataPoint : INotifyPropertyChanged
	{
		...
		private XDataType m_code = XDataType.ASCII;
		...
		/// <param name="text">Text value.</param>
		public DataPoint(XDataType code, string text) : this(text)
		{
			Code = code;
		}
		...
		public DataPoint(XDataType code, string text, ObjectId objectId) : this(code, text)
		{
            ObjectId = objectId;
		}
		...
		public override bool Equals(object obj)
		{
			...
			{
				return m_text.Equals(obj) 
					&& Code == XDataType.ASCII;
			}
			else if (obj.GetType() == typeof(DataPoint))
			{
                DataPoint class1 = (DataPoint)obj;
				return class1.Text == Text 
					&& class1.Code == Code
					&& class1.ObjectId.Equals(ObjectId);
			}
			else
            {
                return false;
		...
		{
			return m_code + " : " + m_text + " : " + ObjectId.ToString();
		}
		...
		public static implicit operator DataPoint(string text)
		{
			return new(text);
		}
		...
		public static explicit operator string(DataPoint attrib)
		{
			return attrib.Text;
		}
		...
		public static bool operator ==(DataPoint class1, DataPoint class2)
		{
		...
		/// <returns><see langword="true"/> if the two objects are equivalent, Otherwise, <see langword="false"/>.</returns>
		public static bool operator ==(DataPoint class1, string class2)
		{
			if (class1 is null && class2 is null)
				return true;
			else if (class1 is null ^ class2 is null)
		...
		/// <returns><see langword="true"/> if the two objects are equivalent; Otherwise, <see langword="false"/>.</returns>
		public static bool operator ==(string class1, DataPoint class2)
		{
			if (class1 is null && class2 is null)
				return true;
			else if (class1 is null ^ class2 is null)
